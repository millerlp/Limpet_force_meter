...
```{r fitShearStationaryData, echo=showcode,dpi=300,fig.width=7,fig.height=7,fig.cap='Forces applied in shear required to dislodge L. gigantea of various aperature areas. The best fit line and 95% prediction interval are shown. Data reproduced from Denny & Blanchette 2000. The dashed horizontal line represents the largest shear force recorded by the captive oystercatcher in this experiment.'}
...
modfit = lm(log(Force.N)~log(Area.m2), data = Dennydata)
Beta = exp(coef(modfit)[1])
alpha = coef(modfit)[2]


newdat = data.frame(Area.m2 = seq(0.00001,0.0018, by = 0.00001))
...
par(family = 'serif')
plot(x=newdat$Area.m2,y = exp(newdat$fitted), type = 'l',
		ylim = c(0,800), las = 1, 
		ylab = 'Force (N)',
		xlab = '',
		xaxt = 'n',
...
xx = c(newdat$Area.m2,rev(newdat$Area.m2))	
...
points(x=Dennydata$Area.m2, y = Dennydata$Force.N, pch = 20)
...
axis(side = 1, at = pretty(newdat$Area.m2), labels = pretty(newdat$Area.m2)*10000)
mtext(side = 1, text = expression(Area~'(cm'^2*')'), line = 2.5)
```

```{r}
# Find the value where the lower 95% prediction interval exceeds the
# maximum horizontal (shearing) force recorded on the transducer (convert to 
# cm^2 from m^2
maxArea.cm2 = newdat$Area.m2[max(which(exp(newdat$lower95) <= 
								maxHorizForce))] * 10000
...
prediction interval, the largest L. gigantea that might be dislodged by a single
pecking force applied in shear (parallel to the substratum) is
`r round(maxArea.cm2,digits = 1)` cm^2. 
