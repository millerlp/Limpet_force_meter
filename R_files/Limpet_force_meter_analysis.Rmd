...
```{r histPecks, echo=showcode, fig.width = 6, fig.height = 8}
par(mfrow=c(2,1)) 
...
```{r plotPeckPushes,echo=showcode, fig.height = 10, fig.width=8,dpi=300,dev='png'}
...
```{r circularAnglePlot,echo=showcode,dpi=300, fig.width=7,fig.height = 7,fig.cap='Angular distribution of force vectors along the anterior-posterior axis and dorsal-ventral axis of the limpet mimic. Each point represents a single strike event by the black oystercatcher during trials on 6 dates. Forces directed anteriorly are plotted on the right side of the plot. Points lying below the horizontal axis indicate forces with a downwards (ventral) vertical component.'}
library(circular)
par(mfrow=c(2,1))
shrinkval = 0.8
panelLetterAdj = 0.2
...
# Body axis labels
axis.circular(template = 'none', units='degrees', zero = pi/2, rotation='clock', 
		labels=c('Anterior','Posterior'), tcl.text = -0.35,
		at = c(circular(c(90,270), units='degrees', template='none')))
axis.circular(template = 'none', units='degrees', zero = pi/2, rotation='clock', 
		labels=c('Right','Left'), tcl.text = -0.5, 
...
points.circular(h, stack = TRUE, bins = 360, shrink =1.3, 
		...
		pch = 20, sep = 0.05)	
par(xpd=FALSE)
mtext(side = 3, text = 'A', cex = 2, adj = panelLetterAdj)
...
points.circular(f, stack = TRUE, bins = 360, shrink =1.3, 
		...
		pch = 20, plot.info = plot2, sep = 0.05)	
mtext(side = 3, text = 'B', cex = 2, adj = panelLetterAdj)

	
# Test points
#points.circular(h[1], col = 'blue',plot.info=thisplot)
#points.circular(circular(0,units='degrees',template='none'), pch = 16, 
#		col = 'green',plot.info=thisplot)
#points.circular(circular(90,units='degrees',template='none'), pch = 16, 
#		col = 'red',plot.info=thisplot)
#points.circular(circular(-123,units='degrees',template='none'), pch = 21, 
#		bg = 'yellow',plot.info=thisplot)

library(png)
img = readPNG("Lgigantea_profile_cutout.png")
rasterImage(img,-0.69,-0.01,0.78,0.35)
# TODO: darken the limpet image, place it better, preserve aspect ratio, make
# image for the top view also.
```
...
library(ggplot2)
# circular histogram of force vectors
ggplot(forceDir, aes(vertAngle)) + geom_histogram(binwidth = 12) + 
				scale_x_continuous(breaks = seq(0,360, by = 30),lim=c(0,360)) + 
				coord_polar(start = pi/2) + xlab(NULL)
