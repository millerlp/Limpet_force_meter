...
#		polygon(greenlocsx,greenlocsy,col='green')
			...
			if (zoom == FALSE & skip == TRUE){
				...
				# Generate a new plot centered around the clickRow
				...
				bluelocsy = c(ylims[2]+20,ylims[2]+20,ylims[2]+100,ylims[2]+100)
				polygon(bluelocsx,bluelocsy,col='lightblue')
				text(x = xlims[1]+1650,y = ylims[2]+50, 
						labels='Skip to next section', adj = 0)
...
#						labels='Done selecting', adj = 0)
				text(x=par()$usr[1], y = par()$usr[4]+10,
						labels='Click baseline section or hit move', adj = 0)
...
#							clickLocation$y < pinklocsy[3]){
				...
				plot(x = temp$Time.msec, y = temp$Norm,type='l')
				# TODO: Have user identify peaks here to be classified as peak/push
				

			} else if (zoom == FALSE & skip == FALSE ){
				# let the plot loop repeat
				repeatLoop = TRUE 
			}
		}
