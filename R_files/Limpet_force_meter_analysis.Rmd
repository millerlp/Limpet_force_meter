...
write.csv(events3, file = './Data/Events_classified_20180213.csv',row.names=FALSE)
...
```{r examplePeckPushPlots,echo=showcode,dpi=600,fig.width=6,fig.height=6,fig.cap='Examples of net forces exerted by the captive bird on the force transducer. Data from 2016-12-14 at 621.5 sec elapsed'}
...
```{r forceHistograms,echo=showcode, dpi=300,fig.width=6,fig.height=6,fig.cap='Histogram of forces in the horizontal plane (A,B) and three dimensions (C,D), classified as pecks or pushes. Left column shows events classified as pecks (n = 218), right column shows events classified as pushes (n = 137). Points and error bars represent the mean +/- 1 SD.'}
...
`r round(peckHorizMean,dig=3)` + `r round(peckHorizSD,dig=3)`.
* For horizontal pushes, the mean + 1SD was 
`r round(pushHorizMean,dig=3)` + `r round(pushHorizSD,dig=3)`.
* For pecks in 3 dimensions, the mean + 1SD was
`r round(peck3DMean,dig=3)` + `r round(peck3DSD,dig=3)`.
* For pushes in 3 dimensions, the mean + 1SD was 
`r round(push3DMean,dig=3)` + `r round(push3DSD,dig=3)`.


```{r, strikeAngles,echo=showcode, eval = FALSE}
# TODO: work on this nonsense
...
# Take all angle data and mirror so that it's all anterior-directed
...
# Convert to a circular object, only downward-directed strikes
forceAngle = circular(vertAngle, units='degrees',
		template = 'none')
# Also generate circular data object for all strikes
allAngles = circular(forceDir$vertAngle, units = 'degrees', template = 'none')




f <- circular(allAngles, units='degrees',
		template = 'none')
...
library(circular, quietly = TRUE)
# All angles are flipped 180 degrees, so that downward-directed angles
# are now reported as degrees above horizontal
# Summary stats for only the downward-directed strikes (n=228)
summary(forceAngle+180)
# Summary stats for all strikes, downwards, horizontal, upwards (n=355)
summary(allAngles+180)
# Unload circular package to avoid interference with mean() and sd() functions
detach('package:circular', unload=TRUE)
```
