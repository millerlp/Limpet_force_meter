...
I wrote an interactive function to let the user pick a trial date and 
go through the raw time series data and attempt to identify potential pecks and
pushes. The script has the user first identify a period of at least 10 samples 
(100 milliseconds) where the baseline signal appears stable, and then the user
...
result, the raw X, Y, Z values, time in milliseconds, and calibrated forces in
the X, Y, and Z axes (plus the euclidean norm of all 3 axes) are available for 
each peak event, along with the starting time value for the baseline. These data
could be used to relocate a chunk of the raw timeseries around each peak event
for further analysis. 


```{r PushPeckPickFunction, eval = FALSE}
...
# Inputs
# trialdate  - A Date object giving the date of the bird trial
# forces - a dataframe containing all of the trial data from each day, imported
# earlier
# calibs - a list object containing the calibration slopes associated from 
# each trial date. Created earlier.
# eventsdf - an optional data frame created from the output of this function. If
# supplied, this will be used to plot markers on already-identified peaks in the
# chosen day's timeseries. 
# baseLength - the number of samples to be averaged together to form the baseline
# values for X, Y, and Z axes prior to an identified peak. 10 samples at 10ms 
# interval = 100 milliseconds
	
peakChooser = function(trialdate, forces = forces, calibs = calibCoeffs, eventsdf = NULL, baseLength=10){
	require(RANN)
	if (!is.null(eventsdf)){
		...
		while(repeatLoop == TRUE){
			...
			if (zoom == FALSE & skip == TRUE){
				...
				text(x = par()$usr[2],y = par()$usr[4], 
						labels='Skip to next section', adj = c(-0.1,-1.5))
...
# write.csv(output, file = 'Events_picker_output_20171208.csv',row.names=FALSE)

```
The output from the `peakChooser` function was saved to a csv file called
`Events_picker_output_20171208.csv`. The data in that file next need to be 
processed to determine if each event was a peck or a push according to the 
criteria outlined above. 

```{r classifyPeckPushFunction}
# A function to create plots of identified events and have the user classify
# them as pecks or pushes.
...
peckpushClassify = function(events, forces, calibs){
...
#	# Get value of next largest force either immediately before or after the
#	# peak
		nearestN = max(c(chunk$Norm[peakIndex-1],chunk$Norm[peakIndex+1]))
#	
		if (chunk$Norm[peakIndex] >= 2) {
			if (chunk$Norm[peakIndex] / nearestN > 2) {
				# If the ratio of the peak force to its nearest neighbors is greater
				# than 2, it meets the threshold for magnitude of a peck
				peckThreshMagnitude = TRUE
			}
			if (chunk$Norm[peakIndex-1] < 2 & chunk$Norm[peakIndex+1] < 2) {
				# If the time points immediately before and after the peak are 
				# both 
				# below 2 newtons, the peak meets the criteria for peck duration
				# because the time above 2N must not exceed 20 ms (=2 samples)
				peckThreshTime = TRUE
			} 
			
			if (peckThreshMagnitude == TRUE & peckThreshTime == TRUE){
				events$PeckPush[i] = 'Peck'
			} else if (peckThreshMagnitude != TRUE | peckThreshTime != TRUE){
				events$PeckPush[i] = 'Push'
			}
		} else if (chunk$Norm[peakIndex] < 2){
			# Peak is less than 2 Newtons, this cannot be a peck or push
			events$PeckPush[i] = NA
		}
...
plotpeckpush = function(eventNum, events, forces, calibs){
	for (j in 1:length(calibCoeffs)){
		if (j == 1){
			calibDates = calibCoeffs[[j]]$Date	
		} else {
	...
	points(x = chunk$Time.msec[1:10], y = chunk$Norm[1:10], pch = 20,
			col = 'orange')
	...
	if (chunk$Norm[peakIndex-1] < 2 & chunk$Norm[peakIndex+1] < 2) {
		# If the time points immediately before and after the peak are 
		# both below 2 newtons, the peak meets the criteria for peck duration
		# because the time above 2N must not exceed 20 ms (=2 samples)
		peckThreshTime = TRUE
	}  else {
		peckThreshTime = FALSE
	}
	...
	# If either neighboring point is higher than this threshold, the peak 
	# cannot be classified as a peck
	mtext(side = 3, text = paste0(trialDate, ', ',peckPush), cex= 1.5, line = 1)
...
events2 = read.csv(paste0(fdir,'Events_picker_output_20171208.csv'))
events2$Date = as.Date(events2$Date)

events3 = peckpushClassify(events = events2, forces = forces, 
		calibs = calibCoeffs)

#write.csv(events3, file = 'Events_classified_20171208.csv',row.names=FALSE)
